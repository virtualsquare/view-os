...
#	endif
#	define FGDEBUG(ofile, level, args...) fgdebug(ofile, level, __FILE__, __LINE__, __func__, args)
#	define GDEBUG(level, args...) FGDEBUG(stderr, level, args)
#	define GPERROR(level, prefix) GDEBUG(level, "%s: %s", prefix, strerror(errno))
#	define FGHEXDUMP(ofile, level, text, len) fghexdump(ofile, level, __FILE__, __LINE__, __func__, text, len)
