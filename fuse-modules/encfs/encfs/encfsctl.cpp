...
template<typename T>
int processContents( const Ptr<EncFS_Root> &rootInfo, const char *path, T &op )
{
    int errCode = 0;
    Ptr<FileNode> node = rootInfo->root->openNode( path, "encfsctl",
	    O_RDONLY, &errCode );
    if(node.isNull())
    {
	...
	// avoid 'leak' error message..
	rootInfo->root->release( path );
    }
    return 0;
} 
...
static int copyContents(const Ptr<EncFS_Root> &rootInfo, 
                        const char* encfsName, const char* targetName)
{
    Ptr<FileNode> node = rootInfo->root->lookupNode( encfsName, "encfsctl" );
...
static int traverseDirs(const Ptr<EncFS_Root> &rootInfo, 
