 ...
 * Copyright (c) 2010 Renzo Davoli (umfuse support)
 *
...
#define DEFINE_ctx
#else
static __inline__ ntfs_fuse_context_t *current_ctx(void) {
	struct fuse_context *mycontext=fuse_get_context();
	ntfs_fuse_context_t *ctx=mycontext->private_data;
	return ctx;
}
#define DEFINE_ctx ntfs_fuse_context_t *ctx = current_ctx()
#endif
...
#if defined(FUSE_CAP_DONT_MASK) || (defined(__APPLE__) || defined(__DARWIN__)) || defined(_UMFUSE)
...
#ifdef _UMFUSE
static ntfs_fuse_context_t *ntfs_fuse_init(void)
{
	ntfs_fuse_context_t *ctx = ntfs_calloc(sizeof(ntfs_fuse_context_t));
	if (ctx) {
...
#ifdef _UMFUSE
static int ntfs_open(ntfs_fuse_context_t *ctx, const char *device)
#else
...
#ifdef _UMFUSE
static char *parse_mount_options(ntfs_fuse_context_t *ctx, 
		const char *orig_opts)
#else
...
#ifdef _UMFUSE
static int set_fuseblk_options(ntfs_fuse_context_t *ctx,
		char **parsed_options)
#else
...
#ifdef _UMFUSE
static struct fuse *mount_fuse(ntfs_fuse_context_t *ctx,
		char *parsed_options)
#else
...
#endif
{
	...
	fh = fuse_new(ctx->fc, &args , &ntfs_3g_ops, sizeof(ntfs_3g_ops), 
#ifdef _UMFUSE
			ctx
#else
			NULL
...
#ifdef _UMFUSE
static void setup_logging(ntfs_fuse_context_t *ctx,
		char *parsed_options)
#else
...
int main(int argc, char *argv[])
{
	...
	parsed_options = parse_mount_options(
#ifdef _UMFUSE
			ctx,
#endif
			opts.options);
	...
	if (ctx->blkdev && set_fuseblk_options(
#ifdef _UMFUSE
				ctx,
#endif
				&parsed_options))
