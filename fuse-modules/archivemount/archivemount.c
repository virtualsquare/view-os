/*
   ...
   Copyright (c) 2010 Renzo Davoli <renzo@cs.unibo.it>, 
	 	new version including umfuse support.
...
typedef struct archive_context {
...
#define AR_OPT(t, p, v) { t, offsetof(struct archive_context, p), v }
...
{
	struct archive_context *ac=data;
	switch( key ) {
...
build_tree( archive_context *ac )
{
	...
	if( archive_read_open_fd( archive, ac->archiveFd, 10240 ) != ARCHIVE_OK ) {
		fprintf( stderr, "%s\n", archive_error_string( archive ) );
		return archive_errno( archive );
	}
	...
	{
		ac->archiveWriteable = 0;
	}
	/* create root node */
	if( (ac->root = malloc( sizeof( NODE ) ) ) == NULL ) {
	        log( "Out of memory" );
		return -ENOMEM;
	}
	init_node( ac->root );
	ac->root->name = strdup( "/" );
	/* fill ac->root->entry */
	if( (ac->root->entry = archive_entry_new()) == NULL ) {
	        log( "Out of memory" );
		return -ENOMEM;
	}
	if( fstat( ac->archiveFd, &st ) != 0 ) {
		perror( "Error stat'ing archiveFile" );
		return errno;
	}
	archive_entry_set_gid( ac->root->entry, getgid() );
	archive_entry_set_uid( ac->root->entry, getuid() );
	archive_entry_set_mode( ac->root->entry, st.st_mtime );
	archive_entry_set_pathname( ac->root->entry, "/" );
	archive_entry_set_size( ac->root->entry, st.st_size );
	stat( ac->mtpt, &st );
	archive_entry_set_mode( ac->root->entry, st.st_mode );
	...
	while( archive_read_next_header( archive, &entry ) == ARCHIVE_OK ) {
		...
		/* references */
		if( insert_by_path( ac->root, cur ) != 0 ) {
	...
	lseek( ac->archiveFd, 0, SEEK_SET );
	return 0;
}

#ifdef _UMFUSE
static void rec_free_tree(NODE *this)
{
	if (this != NULL) {
		rec_free_tree(this->child);
		rec_free_tree(this->next);
		if (this->name != NULL)
			free(this->name);
...
static void
free_tree( archive_context *ac )
{
	rec_free_tree(ac->root);
}
#endif
...
static int
rename_recursively( archive_context *ac, NODE *start, const char *from, const char *to )
{
	char *individualName;
	char *newName;
	int ret = 0;
	...
	while( node ) {
		if( node->child ) {
			/* recurse */
			ret = rename_recursively( ac, node->child, from, to );
		}
		/* change node name */
		...
		correct_hardlinks_to_node( ac->root, node->name, newName );
		free( node->name );
		node->name = newName;
		node->namechanged = 1;
		/* iterate */
...
save( archive_context *ac )
{
	...
	oldfilename = malloc( strlen( ac->archiveFile ) + 5 + 1 );
	if( !oldfilename ) {
	...
	/* rename old archive */
	sprintf( oldfilename, "%s.orig", ac->archiveFile );
	close( ac->archiveFd );
	if( rename( ac->archiveFile, oldfilename ) < 0 ) {
		int err = errno;
		char *buf = NULL;
		...
		log( "Could not rename old archive file (%s/%s): %s",
				buf, ac->archiveFile, strerror( err ) );
		free( buf );
		ac->archiveFd = open( ac->archiveFile, O_RDONLY );
		return 0 - err;
	}
	ac->archiveFd = open( oldfilename, O_RDONLY );
	free( oldfilename );
	/* open old archive */
	...
	if( archive_read_open_fd( oldarc, ac->archiveFd, 10240 ) != ARCHIVE_OK ) {
		log( "%s", archive_error_string( oldarc ) );
		return archive_errno( oldarc );
	}
	...
	*/
	...
	while( archive_read_next_header( oldarc, &entry ) == ARCHIVE_OK ) {
		...
		name = archive_entry_pathname( entry );
		node = get_node_for_entry( ac->root, entry );
		if( ! node ) {
	...
	while( ( node = find_modified_node( ac->root ) ) ) {
...
static int
_ar_read( archive_context *ac, const char *path, char *buf, size_t size, off_t offset,
		struct fuse_file_info *fi )
{
	...
	/* find node */
	node = get_node_for_path( ac->root, path );
	if( ! node ) {
		return -ENOENT;
	}
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_read( ac, archive_entry_hardlink( node->entry ),
				buf, size, offset, fi );
	}
	if( archive_entry_symlink( node->entry ) ) {
		/* file is a symlink, recurse into it */
		return _ar_read( ac, archive_entry_symlink( node->entry ),
				buf, size, offset, fi );
	}
	if( node->modified ) {
...
ar_read( const char *path, char *buf, size_t size, off_t offset,
		struct fuse_file_info *fi )
{
	archive_context *ac=fuse_get_context()->private_data;
	int ret;
	pthread_rwlock_rdlock(&ac->lock);
	ret = _ar_read( ac, path, buf, size, offset, fi );
	pthread_rwlock_unlock(&ac->lock);
	return ret;
}

static int
_ar_getattr( archive_context *ac, const char *path, struct stat *stbuf )
{
	NODE *node;
	int ret;
	...
	if( archive_entry_hardlink( node->entry ) ) {
		...
		ret = _ar_getattr( ac, archive_entry_hardlink(
					node->entry ), stbuf );
		return ret;
	}
...
ar_mkdir( const char *path, mode_t mode )
{
	...
	if( ! ac->archiveWriteable || ac->readonly ) {
		return -EROFS;
	}
	pthread_rwlock_wrlock( &ac->lock );
	...
	if( ac->root->child &&
			node->name[0] == '/' &&
			archive_entry_pathname( ac->root->child->entry )[0] != '/' )
	{
	...
	/* add node to tree */
	if( insert_by_path( ac->root, node ) != 0 ) {
		log( "ERROR: could not insert %s into tree",
				node->name );
		rmdir( location );
		free( location );
...
ar_symlink( const char *from, const char *to )
{
	...
	pthread_rwlock_wrlock( &ac->lock );
	/* check for existing node */
	node = get_node_for_path( ac->root, to );
	if( node ) {
		pthread_rwlock_unlock( &ac->lock );
		return -EEXIST;
	}
...
ar_link( const char *from, const char *to )
{
	...
	pthread_rwlock_wrlock( &ac->lock );
	/* find source node */
	fromnode = get_node_for_path( ac->root, from );
	if( ! fromnode ) {
		pthread_rwlock_unlock( &ac->lock );
		return -ENOENT;
	}
...
static int
_ar_truncate( archive_context *ac, const char *path, off_t size )
{
	NODE *node;
	char *location;
	int ret;
	...
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_truncate( ac, archive_entry_hardlink(
					node->entry ), size );
	}
	if( archive_entry_symlink( node->entry ) ) {
		/* file is a symlink, recurse into it */
		return _ar_truncate( ac, archive_entry_symlink(
					node->entry ), size );
	}
	if( node->location ) {
		...
		while( tmpsize ) {
			...
			/* read */
			if( ( tmp = _ar_read( ac, path, tmpbuf, len, tmpoffset, &fi ) )
					< 0 )
			{
...
ar_truncate( const char *path, off_t size )
{
	...
	int ret;
	pthread_rwlock_wrlock( &ac->lock );
	ret = _ar_truncate( ac, path, size );
	pthread_rwlock_unlock( &ac->lock );
	return ret;
}

static int
_ar_write( archive_context *ac, const char *path, const char *buf, size_t size,
		off_t offset, struct fuse_file_info *fi )
{
	...
	if( S_ISLNK( archive_entry_mode( node->entry ) ) ) {
		/* file is a symlink, recurse into it */
		return _ar_write( ac, archive_entry_symlink( node->entry ),
				buf, size, offset, fi );
	}
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_write( ac, archive_entry_hardlink( node->entry ),
				buf, size, offset, fi );
	}
	if( archive_entry_symlink( node->entry ) ) {
		/* file is a symlink, recurse into it */
		...
		off_t offset, struct fuse_file_info *fi )
{
	...
	int ret;
	pthread_rwlock_wrlock(&ac->lock);
	ret = _ar_write( ac, path, buf, size, offset, fi );
	pthread_rwlock_unlock(&ac->lock);
	return ret;
}
...
static int
_ar_chmod( archive_context *ac, const char *path, mode_t mode )
{
	NODE *node;
	...
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_chmod( ac, archive_entry_hardlink( node->entry ), mode );
	}
	if( archive_entry_symlink( node->entry ) ) {
		/* file is a symlink, recurse into it */
		return _ar_chmod( ac, archive_entry_symlink( node->entry ), mode );
	}
#ifdef __APPLE__
...
ar_chmod( const char *path, mode_t mode )
{
	...
	int ret;
	pthread_rwlock_wrlock( &ac->lock );
	ret = _ar_chmod( ac, path, mode );
	pthread_rwlock_unlock( &ac->lock );
	return ret;
}

static int
_ar_chown( archive_context *ac, const char *path, uid_t uid, gid_t gid )
{
	NODE *node;
	...
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_chown( ac, archive_entry_hardlink( node->entry ),
				uid, gid );
	}
...
ar_chown( const char *path, uid_t uid, gid_t gid )
{
	...
	int ret;
	pthread_rwlock_wrlock( &ac->lock );
	ret = _ar_chown( ac, path, uid, gid );
	pthread_rwlock_unlock( &ac->lock );
	return ret;
}

static int
_ar_utime( archive_context *ac, const char *path, struct utimbuf *buf )
{
	NODE *node;
	...
	if( archive_entry_hardlink( node->entry ) ) {
		/* file is a hardlink, recurse into it */
		return _ar_utime( ac, archive_entry_hardlink( node->entry ), buf );
	}
	if( archive_entry_symlink( node->entry ) ) {
		/* file is a symlink, recurse into it */
		return _ar_utime( ac, archive_entry_symlink( node->entry ), buf );
	}
...
ar_rename( const char *from, const char *to )
{
	...
	pthread_rwlock_wrlock(&ac->lock);
	node = get_node_for_path( ac->root, from );
	if( ! node ) {
		pthread_rwlock_unlock( &ac->lock );
		return -ENOENT;
	}
	...
	node->namechanged = 1;
	remove_child( node );
	ret = insert_by_path( ac->root, node );
	correct_hardlinks_to_node( ac->root, old_name, node->name );
	free( old_name );
	ac->archiveModified = 1;
	pthread_rwlock_unlock( &ac->lock );
	return ret;
...
static void* ar_init(struct fuse_conn_info *conn) {
...
main( int argc, char **argv )
{
	...
	/* parse cmdline args */
	if( fuse_opt_parse( &args, &ac, ar_opts, ar_opt_proc ) == -1 )
                return -1;
	if( ac.archiveFile==NULL ) {
	...
	ac.archiveFd = open( ac.archiveFile, O_RDWR );
	if( ac.archiveFd != -1 ) {
		ac.archiveWriteable = 1;
		close( ac.archiveFd );
	}
	...
	pthread_rwlock_init(&ac.lock, NULL);

	/* now do the real mount */
	fuse_ret = fuse_main( args.argc, args.argv, &ar_oper, &ac );

	/* go back to saved dir */
	fchdir( oldpwd );

	/* save changes if modified */
	if( ac.archiveWriteable && !ac.readonly && ac.archiveModified ) {
		if( save( &ac ) != 0 ) {
